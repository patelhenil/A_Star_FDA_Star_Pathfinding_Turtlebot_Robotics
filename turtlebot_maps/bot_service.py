...
def main(choice):
	...
	with open("/home/duy/catkin_ws/src/comprobfall2018-hw1/turtlebot_maps/map_3.txt") as f:
		for line in f:
			line = line.replace("(", "")
			line = line.replace(")", "")
			line = line.replace("\n", "")
	...
	for row in range(0, int(matrixHeight) * 4):
	    rowArr = []
	    for col in range(0, int(matrixWidth) * 4):
		rowArr.append(1)
	    matrix.append(rowArr)

	for x in range(0,int(matrixHeight) * 4):
	    matrix[x][0] = 0
	    matrix[int(matrixHeight) * 4-1][x] = 0
	    matrix[x][int(matrixHeight) * 4-1] = 0
	    matrix[0][x] = 0

	for y in range(0, int(matrixHeight) * 4):
	    for x in range(0, int(matrixWidth) * 4):
		if y + 1 >= matrixHeight * 4 or x + 1 >= matrixWidth * 4:
		    continue
	...
	for coor in path:
		x = float(coor[0])/4 - offset_x
		y = float(coor[1])/4 - offset_y
